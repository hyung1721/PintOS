...
buffer_cache_init (void)
{
    ...
    /* Create a thread which does write-behind. */
...
get_buffer_cache_index (block_sector_t sector)
{
 ...
 struct cache_slot* cache_slot_alloc (void)
 {
    while (1)
    {
        ...
        else
        {
            ...
            if (current_slot->dirty)
            {
                ...
                //lock_release (&buffer_cache_lock[current_slot->index]);
            }            
            current_slot->used = false;
            current_slot->accessed = false;
            current_slot->dirty = false;
            // update current_slot->data??
